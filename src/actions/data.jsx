...
export function storeTask(token, task, updateTaskIdCallback) { //Pass a function in to callback and update the task list?
  return dispatch => {
    dispatch(storeTasksRequest());
    store_task(token, task.email, task.task, task.status)
      .then(parseJSON)
      .then(response => {
